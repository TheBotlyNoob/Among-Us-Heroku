...
const proxy = spawn(`${__dirname}/proxy.exe`, { cwd: __dirname });
...
async function claimPlayit(url, type, port, file) {
  if (url instanceof String || type instanceof String || port instanceof Number)
    throw new Error(
      'URL Must Be A String, Type Must Be A String, And The Port Must Be A Number!'
    );
